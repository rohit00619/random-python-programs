...
cur.execute('''CREATE TABLE Twitter(name TEXT, retrieved INTEGER, friends, INTEGER)''')
...
while True:
    acct=input('Enter a Twitter account, orquit:')
    if(acct=='quit'): break
    if(len(acct)<1):
...
for u in js['users']:
    ...
    try:
        ...
        cur.execute('''INSERT INTO Twitter(name,retrieved,friends)VALUES(?,0,1)''')
        countnew=countnew+1
